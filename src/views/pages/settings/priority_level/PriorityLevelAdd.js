...
function PriorityLevelAdd(props) {
    ...
    useEffect(() => {
        ...
        if (deployment && deployment !== undefined) {
            ...
            setLevel(JSON.parse(deployment).priority_level);
    ...
    function validateformData(formData, setInvalidFields) {
        ...
        if (!formData.name) {
            invalidFields.push('PriorityLevel Name');
        }
    ...
    const addRecordInstance = async (data) => {
        ...
        setPending(true);
        const results = await axiosInstance.post('createPriorityLevel',
            JSON.stringify(data),
            {
                headers: {
        ...
        if (results?.data?.status == "success") {
            ...
            dispatch(toggleToaster({ isOpen: true, toasterData: { type: "success", msg: "Priority Level Added Successfully" } }))
    ...
    const updateRecordInstance = async (data) => {
        ...
        setPending(true);
        const results = await axiosInstance.post('updatePriorityLevel',
            JSON.stringify(data),
            {
                headers: {
                    'Content-Type': 'application/json',
        ...
        if (results?.data?.status == "success") {
            ...
            dispatch(toggleToaster({ isOpen: true, toasterData: { type: "success", msg: "Priority Level Updated Successfully" } }))
    ...
    const deleteRecord = async (idD) => {
        setPending(true);
        const results = await axiosInstance.post('deletePriorityLevel',
                ...
                <Card.Header>
                    <Card.Title as="h4">
                        ...
                        <div className="flex items-start justify-between">
                            <span className="my-font-family-overpass-mono font-semibold text-[#dbdbde]">Priority Level : <span className="text-[0.6em] capitalize"> {props?.formType} </span> </span>
                ...
                <Card.Body >
                    ...
                    >
                        ...
                        <div className="md:min-h-[450px] relative">
                            ...
                            <div>
                                <div className="grid gap-6 mb-6 md:grid-cols-2 pt-0">
                                    <div className="mb-4">
                                        <label
                                    ...
                                    <div className="mb-4">
                                        <label htmlFor="type" className="block mb-2 my-label">Level</label>
                                ...
                                <div className="mb-4">
                                    <label for="name" className="block mb-2 my-label">Priority Level Name</label>
                                    <input type="text" onChange={handleChange} name="name" value={formValue.name} id="name" className=" block w-full my-input" placeholder="Name of Priority Level" required />
                                </div>

                                <div className="mb-4">
                                    <label for="description" className="block mb-2 my-label">Describe the PriorityLevel</label>
                                    <textarea id="description" onChange={handleChange} name="description" value={formValue.description} rows="3" class="block p-2.5 w-full my-input" placeholder="Priority Level Description..."></textarea>
...
export default PriorityLevelAdd;
