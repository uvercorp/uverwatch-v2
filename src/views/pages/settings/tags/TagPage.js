...
function TagPage(props) {
  ...
  const updateListRecord = (updatedRecord)=>{
    ...
    const index = tags.findIndex((item) => item?.id === updatedRecord.id);
    setTags((prevList) => prevList.map((item, i) => i === index ? updatedRecord : item));
 ...
 return (
   <>
 {currentPage == 'list' &&
     <TagList deploymentId={deploymentId} toggleFormType={toggleFormType} pending={pending} setPending={setPending} tags={tags} currentPage={currentPage} setCurrentPage={setCurrentPage}/>
    }
    {currentPage == 'add' &&
     <TagAdd deploymentId={deploymentId} toggleFormType={toggleFormType} formType={formType} setFormType={setFormType} record={selectedRecord} pending={pending} setPending={setPending} tags={tags} currentPage={currentPage} setCurrentPage={setCurrentPage} populateList={populateList} updateListRecord={updateListRecord} updateListRecordDelete={updateListRecordDelete}/>
    }    
   </>
 );
}
