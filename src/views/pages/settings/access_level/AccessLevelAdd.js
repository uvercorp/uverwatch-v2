...
function AccessLevelAdd(props) {
    ...
    function validateformData(formData, setInvalidFields) {
        ...
        if (!formData.name) {
            invalidFields.push('AccessLevel Name');
        }
    ...
    const addRecordInstance = async (data) => {
        ...
        setPending(true);
        const results = await axiosInstance.post('createAccessLevel',
            JSON.stringify(data),
            {
                headers: {
        ...
        if (results?.data?.status == "success") {
            ...
            dispatch(toggleToaster({ isOpen: true, toasterData: { type: "success", msg: "Access Level Added Successfully" } }))
    ...
    const updateRecordInstance = async (data) => {
        ...
        setPending(true);
        const results = await axiosInstance.post('updateAccessLevel',
            JSON.stringify(data),
            {
                headers: {
                    'Content-Type': 'application/json',
        ...
        if (results?.data?.status == "success") {
            ...
            dispatch(toggleToaster({ isOpen: true, toasterData: { type: "success", msg: "Access Level Updated Successfully" } }))
    ...
    const deleteRecord = async (idD) => {
        setPending(true);
        const results = await axiosInstance.post('deleteAccessLevel',
                ...
                <Card.Header>
                    <Card.Title as="h4">
                        ...
                        <div className="flex items-start justify-between">
                            <span className="my-font-family-overpass-mono font-semibold text-[#dbdbde]">Access Level : <span className="text-[0.6em] capitalize"> {props?.formType} </span> </span>
                    ...
                    >
                        ...
                        <div className="md:min-h-[450px] relative">
                            ...
                            <div>
                                ...
                                <div className="mb-4">
                                    <label for="name" className="block mb-2 my-label">Access Level Name</label>
                                    <input type="text" onChange={handleChange} name="name" value={formValue.name} id="name" className=" block w-full my-input" placeholder="Name of Access Level" required />
                                </div>

                                <div className="mb-4">
                                    <label for="description" className="block mb-2 my-label">Describe the AccessLevel</label>
                                    <textarea id="description" onChange={handleChange} name="description" value={formValue.description} rows="3" class="block p-2.5 w-full my-input" placeholder="Access Level Description..."></textarea>
