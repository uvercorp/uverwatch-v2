...
import ImpactLevelList from "./ImpactLevelList";
import ImpactLevelAdd from "./ImpactLevelAdd";

function ImpactLevelPage(props) {
   const [impactlevels, setImpactLevel] = useState([]);
   ...
   const getImpactLevelData = async (deployment_id) => {
     ...
     setImpactLevel([]);
     try {
       const response = await axiosInstance.get('getImpactLevel/' + deployment_id,
         {
           headers: {
  ...
  return (
    ...
    {currentPage == 'add' &&
     <ImpactLevelAdd accessLevel = {4} deploymentId={deploymentId} toggleFormType={toggleFormType} record={selectedRecord} pending={pending} setPending={setPending} formType={formType} setFormType={setFormType} impactlevels={impactlevels} currentPage={currentPage} setCurrentPage={setCurrentPage} populateList={populateList} updateListRecord={updateListRecord} updateListRecordDelete={updateListRecordDelete}/>
    }

              {/* Close Button */}
              {/* <button
                onClick={() => {setImpactLevel([]);props.setShow(false)}}
...
export default ImpactLevelPage;