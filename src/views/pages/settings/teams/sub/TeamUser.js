...
function TeamUser(props) {
  ...
  useEffect(() => {
    ...
    if (deployment && deployment !== undefined) {
      fetchTeamUserLookup(JSON.parse(deployment).id, props.teamId);
    } else {
  ...
  const fetchTeamUserLookup = async (deployment_id, team_id) => {
    setPending(true);
    try {
      const response = await axiosInstance.get(
        `getTeamUserLookups/${deployment_id}/${team_id}`,
        {
          headers: {
  ...
  const removeUserFromTeam = async (userId) => {
    swal({
      ...
      text: "Are you sure you want to remove this user from the team?",
      icon: "warning",
    ...
    <div className="max-w-6xl mx-auto bg-white rounded-lg">
      ...
      <div className="mb-8">
        ...
        {currentUsers?.length === 0 ? (
          <p className="text-gray-500">No users in this team yet</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {currentUsers?.map((user) => (
              ...
              >
                ...
                <button
                  onClick={() => removeUserFromTeam(user.user)}
