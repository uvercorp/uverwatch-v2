...
const DropdownMenu = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div
      ref={dropdownRef}
      className="absolute right-0 mt-2 w-48 bg-gray-800 border border-gray-600 text-sm z-10 rounded shadow"
    >
      {children}
    </div>
  );
...
function SinglePostListCardForDashboard(props) {
  ...
  return (
    ...
    return ( */}
      <div
        className={`relative p-3 pt-1   text-white transition-colors duration-300 bg-[#1F2F3F] hover:bg-[#3F1F2F] max-h-[220px] min-h[220px] ${
          props?.rightOpen ? "mb-3" : "mb-2  "
        }`}
      ...
      >
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="relative">
            <button
              onClick={() => props.onMenuToggle(props.index)}
            ...
            >
              ...
              {props?.post?.status === "review" && (''
                ...
                //     className="flex items-start gap-2"
                //     onClick={() => {
                //       props?.updatePostStatus(props?.post, "published");
                //     }}
                //   >
                //     <IoArrowUpOutline /> <span>Publish</span>
                //   </div>
                // </button>
              )}
              {props?.post?.status === "published" && (''
                ...
                //     className="flex items-start gap-2"
                //     onClick={() => {
                //       props?.updatePostStatus(props?.post, "review");
                //     }}
                //   >
                //     <IoArrowUpOutline /> <span>Put Under Review</span>
                //   </div>
                // </button>
              )}
              ...
              {currentUser !== "" &&
                ...
                parseInt(props?.post?.created_by) === parseInt(currentUser) && (
                  <>
                    <button className="block w-full text-left px-3 py-2 hover:bg-gray-700">
                    <div className="flex items-start gap-2" onClick={() => { props.setSelectedRecord(props?.post); props?.onAssign(props?.post) }}>
                    <i className="far fa-user"></i> <span>Assign</span></div>
                    </button>
                    <button className="block w-full text-left px-3 py-2 hover:bg-gray-700">
                    <div className="flex items-start gap-2" onClick={() => {props.setSelectedRecord(props?.post); props?.onLink(props.post) }}>
                            <i className="fas fa-exchange-alt"></i>
                            <span>Link</span></div>
                    </button>
                    <button className="block w-full text-left px-3 py-2 hover:bg-gray-700">
                    <div className="flex items-start gap-2"  onClick={() => { props?.setSelectedRecord(props?.post); props.setShow(true);  props?.onEdit(props?.post) }}>
        ...
        <div className="text-sm text-gray-400 mb-1">
          <span className="mr-4">
    DATE: {new Date(props?.post?.created_at).toLocaleDateString('en-GB', {
      day: '2-digit',
      month: '2-digit',
      year: '2-digit'
    }).replace(/\//g, '.')}
          ...
          <span className="mr-4">TIME: {new Date(props?.post?.created_at).getHours().toString().padStart(2, '0')}{new Date(props?.post?.created_at).getMinutes().toString().padStart(2, '0')}HRS</span>
          <span className="mr-4">📂 {props?.post?.is_entity === "true" ? props?.post?.entity_type_name : props?.post?.category_name} </span>
        </div>

        {/* Summary */}
        {/* <p className="text-sm mb-2">{props?.post?.description}</p> */}
        <p className="text-sm mb-2 truncate w-full">{props?.post?.description}</p>

        {/* Coordinates */}
        <p className="text-xs text-gray-400 mb-1 truncate w-full">{props?.post?.formatted_address}, <span className="mr-1">{props?.post?.longitude},</span> <span className="mr-1">{props?.post?.latitude}</span></p>

        {/* Status */}
        <div className="text-xs mb-1">
          {/* {status.map((item, index) => ( */}
            <span className="mr-2 font-semibold capitalize truncate w-full">{props?.post?.status}  <span className="text-xs text-gray-300">: {props?.post?.post_status_name}</span></span>
          {/* ))} */}
          {/* statuse */}
        </div>
...
export default SinglePostListCardForDashboard;