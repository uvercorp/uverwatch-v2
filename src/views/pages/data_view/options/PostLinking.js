...
const PostLinking = ({ post, posts, userAccess }) => {
    ...
    try {
      ...
      if (response?.data?.status === "success") {
        dispatch(
          toggleToaster({
            isOpen: true,
            toasterData: { type: "success", msg: "Link Created successfully." },
          })
        );
        // Update local state
        setExistingLinks([
  ...
  return (
    <div className="container-fluid md:min-h-[500px] md:max-h-[500px] my-gradient-bg">
      <div className="row">
        {/* Current Post */}
        <div className="col-md-4 border-right p-0 pr-1">
          <div className="p-2 bg-[#1F2F3F] hover:bg-[#3F1F2F]">
            <h2 className="mb-2 mt-0 text-lg font-semibold text-gray-400 dark:text-white">
              <i
          ...
          <div className="mb-3 mt-2">
            ...
            {existingLinks.map((link) => (
              <div key={link.id} className=" py-2 bg-[#3F1F2F] p-2">
                <i
                  className={`${link?.icon || "fas fa-question-circle"} text-${
                    post?.color || "muted"
                  }`}
                  style={{
                ...
                />
                <span className="text-amber-600"> Linked to:</span>{" "}
                <span className="text-gray-300">
                  {posts.find((p) => p.id === link.linked_to)?.title}
                </span>
                <br />
                <small>
                  <span className="text-amber-600">Relationship: </span>
                  <span className="text-gray-300">{link.how}</span>
                </small>
                <br />
                <small>
                  <span className="text-amber-600">Assessment: </span>
                  <span className="text-gray-300">{link.assessment}</span>
                </small>
              </div>
            ))}
          </div>
        ...
        <div className="col-md-8">
          <div className="mb-4">
            <input
              type="text"
              className="block w-full my-input"
              placeholder="Search Reports to link..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <small className="form-text text-muted flex items-start justify-between">
              <div>Start typing to search available Reports</div>
              <div>Results {filteredPosts.length}</div>
            </small>
          </div>
          {pending && (
            ...
            <div className="list-group">
              {filteredPosts.map((targetPost) => {
                const isLinked = existingLinks.some(
                  (link) => link.linked_to === targetPost.id
                );
                const hasAccess = targetPost.access_level <= userAccess;

                return (
                  <div key={targetPost.id} className="  bg-[#1F2F3F] hover:bg-[#3F1F2F] p-2 mb-1">
                    <div className="grid grid-cols-1">
                      <div >
                        <h5>
                          <i
                            className={`truncate w-full ${
                              targetPost?.icon || "fas fa-question-circle"
                            } text-${targetPost?.color || "muted"}`}
                            style={{
                              color: isValidColor(targetPost?.color)
                                ? targetPost?.color
                                : "#6c757d",
                              fontSize: "1.2rem",
                              marginRight: "8px",
                        ...
                        <p className="mb-1 text-[0.84em] truncate w-full">
                      ...
                      <div className="pl-2 flex items-start justify-between gap-4">
                        ...
                        {hasAccess && !isLinked && (
                          <>
                            <label className="w-[8%]">How</label>
                            <div className="w-[90%] flex item-start">
                              <input
                                type="text"
