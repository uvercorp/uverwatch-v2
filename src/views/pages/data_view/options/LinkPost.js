  ...
  const getCollectionData = async (deployment_id) => {
    ...
    try {
      const response = await axiosInstance.get(
        "getDeploymentCollection/" + deployment_id,
        {
          headers: {
    ...
    if (checkedCollections.length === 0) {
      dispatch(
        toggleToaster({
          ...
          toasterData: {
            type: "error",
            msg: "Please select at least one collection.",
          },
        })
      );
      return;
      ...
      if (response?.data?.status === "success") {
        dispatch(
          toggleToaster({
            ...
            toasterData: {
              type: "success",
              msg: "Post added to collections successfully.",
            },
          })
        );
    ...
    <>
      ...
      <AnimatePresence>
        {props.show && (
          ...
          >
            <motion.div
              className="my-gradient-bg  shadow-lg md:w-[65%] max-w-[1200px] p-8 px-2 pl-4 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900 max-h-[90vh] z-[1201]"
            ...
            >
              <div className="flex items-start justify-between">
                <span className="text-[1.4em] my-font-family-overpass-mono font-semibold text-[#dbdbde]">
                  Link Report / Entity
                </span>
              </div>
              <div className="px-4">
