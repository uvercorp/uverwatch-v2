...
//   }, [post?.id]);
    ...
    swal({
      ...
      if(confirm) {
        try {
          ...
          const unassignedUser = currentUserAssignments
            .find(a => a.user.id === userId)?.user;
    ...
    swal({
      ...
      if(confirm) {
        try {
          ...
          const unassignedTeam = currentTeamAssignments
            .find(a => a.team.id === teamId)?.team;
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white rounded-lg p-0">
      ...
      <div className="list-group-item mb-4">
        ...
        /> <span className="font-bold">{post.title}</span></h5>
