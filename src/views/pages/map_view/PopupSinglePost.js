import React, { useEffect, useState } from "react";
import { useHistory } from "react-router-dom";

import { CiEdit } from "react-icons/ci";
import { FaShare } from "react-icons/fa";
import { format } from 'timeago.js';
// import { selectedSingleData, setSelectedSingleData } from 'provider/features/helperSlice';
// import { useSelector, useDispatch } from 'react-redux';

function PopupSinglePost(props) {
  const myImage = "data:image/png;base64,"
  const [image, setImage] = useState();
  const [displayName, setDisplayName] = useState();
  // const dispatch = useDispatch();
  let navigate = useHistory();
  const isValidColor = (color) => {
    const style = new Option().style;
    style.color = color;
    return style.color !== '';
  };
  useEffect(() => {
    let dp = localStorage.getItem('deployment');
    let deployment = localStorage.getItem('settings');
    if (deployment && deployment !== undefined) {

      setImage(JSON.parse(deployment).logo);
      setDisplayName(JSON.parse(dp).display_name);
    }

  }, []);


  return (
    // <div className="min-h-[50px] bg-white rounded shadow-sm  grid grid-cols-1 gap-1 items-start justify-start py-2" style={{ border:'1px solid #f5f5f5', borderLeft: '10px solid black' , width:"500px"}}>

    <div
    className={`relative p-3 pt-1   text-white transition-colors duration-300 bg-[#1F2F3F] hover:bg-[#3F1F2F] max-h-[220px] min-h[220px]  "
    }`}
    style={{ border: "1px solid #f5f5f5", borderLeft: "10px solid black", width: "500px" }}
    >
      <div className="flex items-start justify-between ">
      <span className="italic"> {props?.post?.is_entity === "true"? "Entity":"Post"}</span>
      <div className="flex justify-end gap-6 px-2">



      </div>
      </div>
      <div className="">
        <h2 className="mb-2 mt-0 text-lg font-semibold text-gray-200 dark:text-white">
          <i

            className={`truncate w-full ${props?.post?.icon || 'fas fa-question-circle'} text-${props?.post?.color || 'muted'}`}
            style={{
              color: isValidColor(props?.post?.color) ? props?.post?.color : '#6c757d',
              fontSize: '1.2rem',
              marginRight: '8px'
            }}
          /> {props?.post?.title}
        </h2>
        <p className="text-[0.84em] truncate w-full">{props?.post?.description}</p>
      </div>
      <div className="flex justify-start gap-6">
        {/* <div className="min-h-[45px] min-w-[45px] rounded-full  shadow flex items-center justify-center">
          <img src={props?.post?.user_type == 'anonymous' ? myImage : image} style={{height:'45px',width:'45px'}}/>
        </div> */}
        <div className="grid grid-cols-1">
          <div>
            {/* {props?.post?.deployment_name} */}
            {props?.post?.user_type == 'anonymous' ? "Anonymous" : displayName}
          </div>
          <div className="flex justify-between gap-6 text-sm text-gray-500">
            <div>{format(props?.post?.created_at)}</div>

            <div className={props?.post?.status == 'review' || props?.post?.status == 'draft' ? 'bg-[#d2e4ea] px-1 rounded-sm capitalize text-black' : 'bg-slate-100 px-1 rounded-sm capitalize'}>

              {props?.post?.status == 'review' || props?.post?.status == 'draft' ? 'Under Review' : 'Published'}
            </div>
            {/* <div>via web</div> */}
            <div className="text-sm cursor-pointer text-white hover:underline " onClick={() => { props?.handleReadMore(props?.post) }}>Read More</div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default PopupSinglePost;